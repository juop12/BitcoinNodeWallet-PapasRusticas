...
pub fn build_utxo_info(utxo_info: &UTxOInfo) -> Box {
    ...
    let hash_as_string =
        get_string_representation_from_bytes(&mut utxo_info.outpoint.hash.to_vec());
    ...
    let index_label = Label::new(Some(
        format!("Index: {}", utxo_info.outpoint.index).as_str(),
    ));
...
pub fn build_pending_tx_info(pending_tx_info: &TxInfo) -> Box {
    ...
    let amount_btc: f64 =
        (pending_tx_info.tx_out_total + pending_tx_info.tx_in_total) as f64 / SATOSHI_TO_BTC;
