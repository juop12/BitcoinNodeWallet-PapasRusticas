...
pub fn update_available_balance(builder: &Builder, amount: &str) {
    let available_label: Label = builder
        .object("Available Quantity")
        .expect("Couldn't find available balance label");
...
pub fn update_sending_pending_balance(builder: &Builder, amount: &str) {
    let pending_label: Label = builder
        .object("Sending Pending Quantity")
        .expect("Couldn't find sending pending balance label");
...
pub fn update_receiving_pending_balance(builder: &Builder, amount: &str) {
    let pending_label: Label = builder
        .object("Receiving Pending Quantity ")
        .expect("Couldn't find receiving pending balance label");
    pending_label.set_label(amount);
    update_total_balance(builder);
}
...
fn update_total_balance(builder: &Builder) {
    let total_label: Label = builder
        ...
        .expect("Couldn't find total balance label");
    let available_label: Label = builder
        .object("Available Quantity")
        .expect("Couldn't find available balance label");
    let receiving_pending_label: Label = builder
        .object("Receiving Pending Quantity ")
        .expect("Couldn't find receiving pending balance label");

    let available_amount: f64 = available_label.label().parse::<f64>().unwrap_or(0.0);

    let receiving_pending_amount: f64 = receiving_pending_label
        .label()
        .parse::<f64>()
        .unwrap_or(0.0);
