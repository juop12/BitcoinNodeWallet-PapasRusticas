...
impl MessageTrait for InvMessage {
    ...
    fn from_bytes(slice: &[u8]) -> Result<Self::MessageType, MessageError> {
        ...
        while i < slice.len() {
            let aux: [u8; INVENTORY_ENTRY_SIZE] =
                match slice[(i)..(i + INVENTORY_ENTRY_SIZE)].try_into() {
                    Ok(array) => array,
...
mod test {
    ...
    #[test]
    fn inv_test3_message_is_created_properly_from_correct_amount_of_bytes(
    ) -> Result<(), MessageError> {
        let (mut expected_bytes, _hash1, _hash2) = inv_message_expected_bytes(false);
